...
<p>名前・レベルで行を、耐性名で列を絞り込めます。● はモンスター定義にある耐性フラグです。弱点フラグ（HURT_*）は含めていません。</p>
<p>Filter rows by name and level, and columns by resistance name. ● denotes a resistance flag in the monster definition; vulnerability flags (HURT_*) are excluded.</p>
...
<p class="resistance-note">※ これは定義ファイルのフラグ一覧です。ゲーム中の個別属性ダメージ計算や複合耐性を網羅した判定表ではありません。 / This lists definition flags, not every derived damage calculation.</p>
<blockquote>🤖 データ元 / Source: <code>lib/edit/MonraceDefinitions.jsonc</code> · hengband <code>${SOURCE_REVISION}</code> · <code>build-monster-resistance.mjs</code></blockquote>
</main></div>
